...
interface TicketPlanFormProps {
  courts: ICourt[];
  modify?: boolean;
}

const TicketPlanForm = ({ courts = [], modify }: TicketPlanFormProps) => {
  const {
    watch,
    register,
    control,
        ...
        <Card>
          <CardContent>
            <MDTypography variant="body3">
              사용자가 구매하여 이용중인 이용권의 경우 일부 내용은 추후 수정이 제한되니 신중하게
              작성해주세요. <br />
              필요 시 이용권을 미노출 후 새로 생성해주세요.
            </MDTypography>

            <Grid container spacing={4} sx={{ display: "flex", flexDirection: "column" }}>
              <Grid item xs={12}>
                <RKLabel>이용권의 이름을 입력해주세요.</RKLabel>
                <MDInput
                  fullWidth
              ...
              <Grid item xs={12}>
                <RKLabel>이 이용권은 어떤 코트에서 사용되나요? (수정 제한)</RKLabel>
                <Controller
                  control={control}
                  name="courts"
              ...
              <Grid item xs={12}>
                <RKLabel>이 이용권의 시간 단위는 1회에 몇 분인가요? (수정 제한)</RKLabel>
                <MDInput
                  fullWidth
                  {...register("maxTime")}
                  defaultValue={30}
              ...
              {/* <Grid item xs={12}>
                <RKLabel>이 이용권은 고객이 한번에 최대 몇 회까지 연속 예약 가능한가요?</RKLabel>
                <MDInput fullWidth />
              </Grid> */}
              {/* <Grid item xs={12}>
                <RKLabel>이 이용권은 고객이 하루에 최대 몇 회까지 예약 가능한가요?</RKLabel>
                <MDInput fullWidth />
              </Grid> */}

              <Grid item xs={12}>
                <RKLabel>이용권 판매 기한 (수정 제한)</RKLabel>
                <Controller
                  control={control}
                  name="expiredTime"
              ...
              <Grid item xs={12}>
                <RKLabel>이용권 총 판매 수량 (수정 제한)</RKLabel>
                <MDBox>
                  <Controller
                    control={control}
                    name="maxQuantity"
              ...
              {/* <Grid item xs={12}>
                <RKLabel>1인당 이용권 구매 수량 제한</RKLabel>
              ...
              <Grid item xs={12}>
                ...
                <MDTypography variant="caption">
                  사용자가 이용권 구매 후 이 시간이 지나야 같은 이용권 재구매가 가능합니다.
                </MDTypography>
              </Grid>
              <Grid item xs={12}>
                <RKLabel>구매 후 이용가능 일수 (수정 제한)</RKLabel>
                <Controller
                  control={control}
                  name="availableDays"
                  render={({ field: { value, onChange } }) => (
                    ...
                    >
                      <MDInput
                        ...
                        disabled={modify}
                        placeholder="숫자만 입력, 최대 180"
                      />
                    </MDBoxWithOptionInfinity>
                  )}
                />
                <MDTypography variant="caption" color="gray">
                  사용자가 구매시점 기준 최대 몇일까지 이용가능한지 설정합니다.
                </MDTypography>
              </Grid>
            </Grid>
          </CardContent>
        ...
        <Card>
          <CardContent>
            <Grid container spacing={4}>
              <Grid item xs={12}>
                <MDTypography variant="body3">
                  이용권의 운영시간을 설정해주세요. 해당 코트의 운영시간을 고려하여 설정해주세요.{" "}
                  <br />
                  이용권의 예약시간은 사용자에게 이번 주 월요일을 기준으로 2주 단위로 노출됩니다.
                </MDTypography>
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item xs={12}>
                <RKLabel>이용권에 대한 설명을 적어주세요</RKLabel>
                <MDInput
                  fullWidth
                  rows={8}
                  multiline
              ...
              <Grid item xs={12}>
                <RKLabel>이용권 가격을 설정해주세요.</RKLabel>
                <MDInput
                  fullWidth
                  type="number"
                  {...register("price", { required: true })}
                  disabled={modify}
                  placeholder="숫자만 입력, 최대 999,000"
                />
              </Grid>
            </Grid>
          </CardContent>
